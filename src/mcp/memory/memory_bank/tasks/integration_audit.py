...
CHECKLIST_PATH = Path('memory_bank/federation_integration_checklist.md')
...
def log_audit(audit_log_path, message):
    with open(audit_log_path, 'a', encoding='utf-8') as f:
        f.write(f'[integration-audit] {message}\n')
...
def create_task_in_tracker(origin, problem_text):
    ...
    if github_token and github_repo:
        ...
        data = {'title': f'[integration][{origin}] {problem_text[:60]}', 'body': problem_text}
        try:
    ...
    if jira_url and jira_user and jira_token and jira_project:
        ...
        data = {
            'fields': {
                ...
                'summary': f'[integration][{origin}] {problem_text[:60]}',
...
def notify_team(origin, summary):
    msg = f'[integration-audit] {origin}: {summary}'
    ...
    if slack_url:
        ...
        try:
            mime = MIMEText(msg)
            mime['Subject'] = f'Integration audit for {origin}'
            mime['From'] = smtp_user
            mime['To'] = email_to
...
def run_integration_audit():
    ...
    for origin_dir in ARCHIVE_ROOT.iterdir():
        ...
        log_audit(audit_log_path, f'Запуск integration audit: {now}')
