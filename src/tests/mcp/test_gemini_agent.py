import pytest
from src.mcp.agents.gemini_agent import GeminiAgent
from unittest.mock import AsyncMock

@pytest.mark.asyncio
async def test_handle_pop_completion():
    mock_client = AsyncMock()
    mock_client.complete.return_value = "Ответ Gemini"
    agent = GeminiAgent(mock_client)
    result = await agent.handle_pop({"prompt": "Привет!"})
    assert result["completion"] == "Ответ Gemini"
