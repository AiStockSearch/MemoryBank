# Project Brief: <Название проекта>

## 1. Мета-данные
- Project ID: <...>
- Владелец: <...>
- Дата старта: <...>
- Версия: <...>
- Статус: draft
- Ссылки: <...>

## 2. Stakeholders & Roles
| Имя | Роль | Ответственность | Контакт |
|-----|------|----------------|---------|
|     |      |                |         |

## 3. Product Strategy & Vision
- Миссия: <...>
- Ценности: <...>
- SWOT: <...>
- Feature matrix: <...>

## 4. Customer Segmentation & Personas
- Сегменты: <...>
- Персоны: <...>
- AI-анализ: <...>

## 5. Customer Intelligence & Deep Insights
- Портреты клиентов: <...>
- Customer Stories: <...>
- AI-анализ отзывов/форумов: <...>
- Customer Success Map: <...>

## 6. Customer Insights & Jobs-to-be-Done
- JTBD: <...>
- Триггеры и барьеры: <...>
- AI-генерация гипотез: <...>

## 7. Product/Market Fit & Validation
- Гипотезы PMF: <...>
- AI-анализ Product/Market Fit: <...>
- Сигналы рынка: <...>

## 8. Customer Acquisition & Growth
- Источники лидов: <...>
- Воронка продаж: <...>
- Growth loops & network effects: <...>
- Виральные механики: <...>
- AI-анализ retention/engagement: <...>
- AI-оценка каналов: <...>

## 9. Customer Value & Monetization
- Средний чек (ARPU): <...>
- LTV: <...>
- Модели монетизации: <...>
- Unit-экономика: <...>

## 10. Market Sizing & TAM/SAM/SOM
- TAM: <...>
- SAM: <...>
- SOM: <...>
- Источники данных: <...>

## 11. Competitive Analysis
- Таблица конкурентов: <...>
- AI-анализ: <...>

## 12. Pricing & Packaging
- Тарифы: <...>
- A/B тесты: <...>
- AI-рекомендации: <...>

## 13. Metrics & Analytics
- Ключевые метрики: <...>
- Динамика: <...>
- AI-прогноз: <...>

## 14. Go-to-Market & Growth Experiments
- План запуска: <...>
- Growth experiments: <...>
- AI-идеи для роста: <...>

## 15. Financial Plan & Forecast
- P&L: <...>
- Unit-экономика: <...>
- Прогноз роста: <...>
- AI-расчёт точки безубыточности: <...>

## 16. Ecosystem & Partnerships
- Карта экосистемы: <...>
- AI-поиск новых партнёров: <...>
- Платформенные возможности: <...>

## 17. Team & Culture
- Командные ценности: <...>
- AI-анализ командных рисков: <...>
- План развития команды: <...>

## 18. Legal & Regulatory
- Юридические риски: <...>
- AI-мониторинг изменений: <...>
- План реагирования: <...>

## 19. Technology & Innovation
- AI/ML roadmap: <...>
- Технологические тренды: <...>
- Open source & community: <...>

## 20. Sustainability & Impact
- Экологический след: <...>
- Социальная миссия: <...>
- AI-оценка ESG-метрик: <...>

## 21. Scenario Planning & Risk Management
- Сценарии развития: <...>
- AI-генерация "what if": <...>
- План реагирования: <...>

## 22. Continuous Discovery & Feedback
- Система сбора обратной связи: <...>
- AI-генерация новых гипотез: <...>
- План discovery: <...>

## 23. Data Strategy & Privacy
- Стратегия данных: <...>
- AI-оценка рисков: <...>
- План монетизации данных: <...>

## 24. Go Global
- Мультиязычность: <...>
- AI-анализ локальных трендов: <...>
- План выхода на новые рынки: <...>

## 25. AI-Driven Automation
- AI-ассистент: <...>
- AI-инициатор: <...>
- Автоматизация: <...>

## 26. Customizable & Extensible
- Модульность: <...>
- Интеграция с внешними системами: <...>
- API-first: <...>

## 27. Visual Dashboards & Live Metrics
- Дашборды: <...>
- Live-отчёты: <...>

## 28. Knowledge Graph & Semantic Links
- Граф знаний: <...>
- AI-поиск по графу: <...>

## 29. AI-Generated Executive Summary
- Ключевые выводы: <...>
- Риски: <...>
- Возможности: <...>
- Прогнозы: <...>

## 30. Hypotheses (Гипотезы)
| Сегмент | Гипотеза | Критерий проверки | Результат теста |
|---------|----------|-------------------|-----------------|
|         |          |                   |                 |

## 31. Market Sizing (TAM/SAM/SOM)
- TAM: <...>
- SAM: <...>
- SOM: <...>
- Источники данных: <...>
- Методика расчёта: <...>
- Целевая доля рынка: <...>
- График динамики рынка: <...>

## 32. Competitors & Killer Features
| Конкурент | Killer features | Причины выбора | Сегменты | Цена |
|-----------|----------------|---------------|----------|------|
|           |                |               |          |      |
- SWOT-конкурентов: <...>
- AI-анализ отзывов конкурентов: <...>
- Customer switching triggers: <...>

## 33. Customer Willingness to Pay
- Опросы: <...>
- A/B тесты: <...>
- Сегментация по доходу: <...>
- Платежные барьеры: <...>
- AI-анализ транзакций: <...>

## 34. Channels & Acquisition
- Карта каналов: <...>
- Эффективность (CAC, LTV): <...>
- Growth loops: <...>
- Customer journey по каналам: <...>
- AI-анализ каналов: <...>

## 35. Customer Validation & Evidence
- Документированные интервью: <...>
- AI-анализ customer feedback: <...>
- Customer advisory board: <...>
- LOI (письма о намерениях): <...>

## 36. Go-to-Market (GTM) & Launch Readiness
- GTM checklist: <...>
- План коммуникаций: <...>
- AI-анализ рисков запуска: <...>

## 37. Retention & Expansion
- Cohort analysis: <...>
- Expansion revenue: <...>
- AI-прогноз churn/retention: <...>

## 38. Product Analytics & Experimentation
- Система экспериментов: <...>
- AI-генерация новых экспериментов: <...>
- Feature adoption metrics: <...>

## 39. Regulatory & Compliance Deep Dive
- Карта регуляторных требований по регионам: <...>
- AI-мониторинг изменений в законах: <...>

## 40. Sustainability & Social Impact (расширено)
- Carbon footprint: <...>
- DEI (Diversity, Equity, Inclusion): <...>
- AI-анализ социальных трендов: <...>

## 41. AI/ML Strategy (углубить)
- AI explainability: <...>
- AI governance: <...>
- AI roadmap: <...>

## 42. Ecosystem Mapping (визуализация)
- Mermaid/Graphviz карта экосистемы: <...>
- AI-анализ влияния экосистемы: <...>

## 43. Knowledge Management
- AI-автоматизация поиска знаний: <...>
- Lessons learned: <...>

## 44. AI-Driven Alerts & Monitoring
- AI-алерты по ключевым метрикам: <...>
- AI-инициатор действий: <...>

## 45. Product Localization & Internationalization
- Локализация интерфейса: <...>
- Адаптация под локальные рынки: <...>
- AI-анализ культурных особенностей: <...>

## 46. Customer Support & Success
- Система поддержки: <...>
- Customer success процессы: <...>
- AI-анализ обращений: <...>

## 47. Brand & Positioning
- Позиционирование на рынке: <...>
- Brand attributes: <...>
- AI-анализ восприятия бренда: <...>

## 48. Pricing Experiments & Elasticity
- Эксперименты с ценами: <...>
- Ценовая эластичность: <...>
- AI-прогноз реакции на изменение цен: <...>

## 49. Partnerships & Alliances
- Ключевые партнёры: <...>
- Стратегические альянсы: <...>
- AI-поиск новых партнёров: <...>

## 50. Intellectual Property & Patents
- Патенты: <...>
- Торговые марки: <...>
- AI-мониторинг нарушений: <...>

---

> AI: Все разделы автоматически проверяются на полноту, связность, актуальность. После генерации — инициировать AI-ревью, экспорт, снапшот. 